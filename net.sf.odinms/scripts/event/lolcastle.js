...
function setup() {
    ...
    for (x = 0; x < createInstances.length; x++) {
        ...
        eim.getMapInstance(mapId, false, false).toggleDrops();
    }
    for (x = 0; x < 5; x++) {
...
function announce() {
    ...
    em.getChannelServer().broadcastPacket(
        net.sf.odinms.tools.MaplePacketCreator.serverNotice(6, "[Event] Field of Judgement will begin in " + i + " minutes"));
    i--;
}
...
function mesoDistribution() {
    ...
    while (iter.hasNext()) {
        ...
        if (eim.getPlayerCount() > 0) {
            ...
            var map = eim.getMapFactory().getInstanceMap(mapId, false, false);
...
function start() {
    scheduleNew();
    em.getChannelServer().broadcastPacket(
        net.sf.odinms.tools.MaplePacketCreator.serverNotice(6, "[Event] Field of Judgement has started! The Doors are now closed."));
    var iter = em.getInstances().iterator();
    while (iter.hasNext()) {
        var eim = iter.next();
        if (eim.getPlayerCount() > 0) {
