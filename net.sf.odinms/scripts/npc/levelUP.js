...
function start() {
    ...
    cm.addText("Welcome to the GM-Helper!#b\r\n");
    cm.addText("#L0#Teleports#l\r\n");
    cm.addText("#L1#Stylist#l\r\n");
    cm.addText("#L2#GM-Shop#l\r\n");
    cm.addText("#L3#Warp<>Players#l\r\n");
    cm.addText("#L4#Get Player IP#l\r\n");
    option = cm.sendSimple();
    if (option == 0) { //Teleports
        cm.addText("Choose a map\r\n");
        cm.addText("#b");
        for (var i = 0; i < maps.length; i++) {
            cm.addText("#L" + i + "##m" + maps[i] + "##l\r\n");
        }
        selectmap = maps[cm.sendSimple()];
        cm.warp(selectmap, 0);
        cm.dispose();
    } else if (option == 1) {//Hair
        cm.addText("Hey there! I could change the way you look! What would you like to change?#b\r\n");
        cm.addText("#L0#Skin#l\r\n");
        cm.addText("#L1#Hair#l\r\n");
        cm.addText("#L2#Hair Color#l\r\n");
        cm.addText("#L3#Eye#l\r\n");
        cm.addText("#L4#Eye Color#l");
        var v1 = cm.sendSimple();
        var v2;
        var current;
        ...
        cm.addText("Please select a channel a character to warp to\r\n");
        cm.addText("  #e#d(CH" + cm.getPlayer().getClient().getChannel() + ")#n#k\r\n");
        ...
        sendTarget = targets[cm.sendSimple()];
        cm.warp(sendTarget.getMap().getId());
        cm.dispose();
    } else if (option == 4) { // Player IP
        var name = cm.sendGetText("Please enter a Player-Name", 0, 0);
        var chr = cm.getChr(name);
        if (chr != null) {
            cm.addText("#b" + chr.getName() + "'s#b#k IP Address: #e");
            cm.addText(cm.getChr(name).getClient().getSession().getRemoteAddress());
        } else {
            cm.addText("#b" +  name  + "#k is #rOffline#k");
        }
        cm.sendOk();
        cm.dispose();
    }
