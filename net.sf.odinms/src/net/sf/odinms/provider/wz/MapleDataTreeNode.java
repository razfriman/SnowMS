/*
 * @(#)MapleDataTreeNode.java	1.24 05/11/17
 *
 ...
 * @author Rob Davis
 */
public class MapleDataTreeNode extends Object implements Cloneable,
		MutableTreeNode, Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 8159210027673950829L;
	/**
	...
	private MapleNodeType type;
	private boolean map;
	private boolean neededToLoad = false;
	private MapleDataProvider dataProv;
	 ...
	 *                   the node's data
	 */
	public MapleDataTreeNode(Object userObject) {
	 ...
	 */
	public MapleDataTreeNode(Object userObject, boolean allowsChildren) {
		super();
		parent = null;
	...
	public void setData(MapleData data) {
		this.data = data;
	}

	public MapleNodeType getType() {
		return type;
	}

	public void setType(MapleNodeType type) {
		this.type = type;
	}
	...
	public boolean isNeededToLoad() {
		return neededToLoad;
	}

	public void setNeededToLoad(boolean neededToLoad) {
		this.neededToLoad = neededToLoad;
	}

	public MapleDataProvider getDataProv() {
		return dataProv;
	}

	public void setDataProv(MapleDataProvider dataProv) {
	 ...
	 */
	public boolean isNodeDescendant(MapleDataTreeNode anotherNode) {
		if (anotherNode == null) {
			return false;
		}
	 ...
	 *		or null if none
	 */
	public TreeNode getSharedAncestor(MapleDataTreeNode aNode) {
		if (aNode == this) {
			return this;
		} else if (aNode == null) {
			return null;
	 ...
	 */
	public boolean isNodeRelated(MapleDataTreeNode aNode) {
	...
	public int getDepth() {
		...
		return ((MapleDataTreeNode) last).getLevel() - getLevel();
	}
	...
	public Object[] getUserObjectPath() {
		...
		for (int counter = 0; counter < realPath.length; counter++) {
			retPath[counter] = ((MapleDataTreeNode) realPath[counter]).getUserObject();
		}
		return retPath;
	}
	 ...
	 */
	public MapleDataTreeNode getNextNode() {
		if (getChildCount() == 0) {
			// No children, so look for nextSibling
			MapleDataTreeNode nextSibling = getNextSibling();

			if (nextSibling == null) {
				MapleDataTreeNode aNode = (MapleDataTreeNode) getParent();
				...
				do {
					...
					aNode = (MapleDataTreeNode) aNode.getParent();
				} while (true);
			} else {
				return nextSibling;
			}
		} else {
			return (MapleDataTreeNode) getChildAt(0);
		}
	}
	 ...
	 */
	public MapleDataTreeNode getPreviousNode() {
		MapleDataTreeNode previousSibling;
		MapleDataTreeNode myParent = (MapleDataTreeNode) getParent();
	...
	public boolean isNodeSibling(TreeNode anotherNode) {
		...
		if (anotherNode == null) {
			...
			if (retval && !((MapleDataTreeNode) getParent()).isNodeChild(anotherNode)) {
	 ...
	 */
	public MapleDataTreeNode getNextSibling() {
		MapleDataTreeNode retval;
		...
		if (myParent == null) {
			...
			retval = (MapleDataTreeNode) myParent.getChildAfter(this);	// linear search
		}
	 ...
	 */
	public MapleDataTreeNode getPreviousSibling() {
		MapleDataTreeNode retval;
		...
		if (myParent == null) {
			...
			retval = (MapleDataTreeNode) myParent.getChildBefore(this);	// linear search
		}
	 ...
	 */
	public MapleDataTreeNode getFirstLeaf() {
		MapleDataTreeNode node = this;

		while (!node.isLeaf()) {
			node = (MapleDataTreeNode) node.getFirstChild();
		}
	 ...
	 */
	public MapleDataTreeNode getLastLeaf() {
		MapleDataTreeNode node = this;

		while (!node.isLeaf()) {
			node = (MapleDataTreeNode) node.getLastChild();
		}
	 ...
	 */
	public MapleDataTreeNode getNextLeaf() {
		MapleDataTreeNode nextSibling;
		MapleDataTreeNode myParent = (MapleDataTreeNode) getParent();
	 ...
	 */
	public MapleDataTreeNode getPreviousLeaf() {
		MapleDataTreeNode previousSibling;
		MapleDataTreeNode myParent = (MapleDataTreeNode) getParent();
	...
	@Override
	public Object clone() {
		MapleDataTreeNode newNode = null;

		try {
			newNode = (MapleDataTreeNode) super.clone();
	...
	public static Comparator MapleDataTreeNodeComparator = new Comparator() {

		public int compare(Object treeNode, Object anotherTreeNode) {
			String name1 = ((MapleDataTreeNode) treeNode).getUserObject().toString();
			String name2 = ((MapleDataTreeNode) anotherTreeNode).getUserObject().toString();
			return name1.compareTo(name2);
		}
	};
} // End of class MapleDataTreeNode