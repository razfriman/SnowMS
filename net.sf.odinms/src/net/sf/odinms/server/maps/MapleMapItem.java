...
public class MapleMapItem extends AbstractMapleMapObject {
	...
	public void sendSpawnData(MapleClient client) {
		if (getMeso() > 0) {
		    client.getSession().write(MaplePacketCreator.dropMesoFromMapObject(displayMeso, getObjectId(), getDropper().getObjectId(), getOwner() != null ? getOwner().getId() : 0, null, getPosition(), (byte) 2, false));
		} else {
		    client.getSession().write(MaplePacketCreator.dropItemFromMapObject(getItem().getItemId(), getObjectId(), 0, getOwner() != null ? getOwner().getId() : 0, null, getPosition(), (byte) 2, false));
		}
	}
}
