...
public class MapleQuestAction {
	...
	public boolean check (MapleCharacter c) {
		switch (type) {
			case MESO:
				...
				//PUT CHANNELSERVER QUEST EXP RATE HERE
	...
	private boolean canGetItem(MapleData item, MapleCharacter c) {
		if (item.getChildByPath("gender") != null) {
			...
			if (gender != 2 && gender != c.getGender().getValue())
				return false;
		}
		...
		switch (type) {
			...
			case QUEST:
				for (MapleData qEntry : data) {
					...
					c.updateQuest(new MapleQuestStatus(MapleQuest.getInstance(questId),
			...
			case FAME:
				...
				if (status.getStatus() == MapleQuestStatus.Status.NOT_STARTED &&
					status.getForfeited() > 0)
					break;
                                int fameGained = MapleDataTool.getInt(data);
				c.addFame(fameGained);
                                c.getClient().getSession().write(MaplePacketCreator.getShowFameGain(fameGained));
