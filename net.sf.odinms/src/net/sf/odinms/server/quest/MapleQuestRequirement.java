		...
		switch (getType()) {
			...
			case ITEM:
				...
				for (MapleData itemEntry : getData().getChildren()) {
					...
					if (quantity < MapleDataTool.getIntConvert(itemEntry.getChildByPath("count")) ||
						MapleDataTool.getIntConvert(itemEntry.getChildByPath("count")) <= 0 && quantity > 0) return false;
				}
				return true;
			case MIN_LEVEL:
			...
			case PET:
				//TODO - player has pet with them
				return false;
		    case MONSTER_BOOK:
			  return c.getMonsterbook().getTotalCount() >= MapleDataTool.getIntConvert(getData());
			default:
				return true;
		}
	}
