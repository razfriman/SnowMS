...
public class MapleInventory implements Iterable<IItem>, InventoryContainer {
	...
	public boolean sortItems() {
    	//TODO add sort inv item support
	    return false;
	}
	
	public boolean mergeItems() {
        //TODO add merge inv item support
	    return false;
	}
	...
	public void move(byte sSlot, byte dSlot, short slotMax) {
		...
		} else if (target.getItemId() == source.getItemId() && !ii.isRechargable(source.getItemId())) {
	...
	public short getItemAmountBySlot(byte slot) {
		short amount = -1;
	...
	public boolean removeItem(byte slot) {
		return removeItem(slot, (short) 1, false);
	}

	public boolean removeItem(byte slot, short quantity, boolean allowZero) {
		IItem item = inventory.get(slot);
