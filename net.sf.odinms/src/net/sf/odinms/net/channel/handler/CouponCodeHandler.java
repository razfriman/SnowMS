...
public class CouponCodeHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		String unk = slea.readMapleAsciiString();
		...
		try {
			validcode = c.getPlayer().isNXCodeValid(code.toUpperCase(), validcode);
		...
		if (validcode) {
			...
			switch (type) {
				...
				case 1:
				case 2:
					c.getPlayer().addCSPoints(type, item);
					break;
				case 3:
					c.getPlayer().addCSPoints(0, item);
					c.getPlayer().addCSPoints(2, (item / 5000));
					break;
				case 4:
					MapleInventoryManipulator.addById(c, item, (short) 1, "An item was obtain from a coupon.");
					c.getSession().write(MaplePacketCreator.rewardCouponCode(item));
					break;
				case 5:
					c.getPlayer().addCSPoints(0, item);
					break;
