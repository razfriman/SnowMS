	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (mode == 1) { // add
			...
			if (ble != null && !ble.isVisible()) {
				c.getSession().write(MaplePacketCreator.buddyListMeesage(13));
			} else if (buddylist.isFull()) {
				c.getSession().write(MaplePacketCreator.buddyListMeesage(11));
			} else {
				try {
					...
					if (charWithId != null) {
						...
						if (buddyAddResult == BuddyAddResult.BUDDYLIST_FULL) {
							c.getSession().write(MaplePacketCreator.buddyListMeesage(12));
						} else {
						...
						c.getSession().write(MaplePacketCreator.buddyListMeesage(15));
					}
