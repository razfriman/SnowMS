...
<%@page import="net.sf.odinms.tools.HexTool"%>
...
mplew.writeInt(Integer.parseInt(request.getParameter("int")));
}
...
if(request.getParameter("byte") != null){
input = request.getParameter("byte");
mplew.write(Integer.parseInt(request.getParameter("byte"), 16));
}
...
ByteBuffer bbuf = ByteBuffer.wrap(mplew.getPacket().getBytes());
bbuf = bbuf.order(ByteOrder.BIG_ENDIAN);
bbuf.flip();
ByteBuffer lbuf = ByteBuffer.wrap(mplew.getPacket().getBytes());
lbuf = lbuf.order(ByteOrder.LITTLE_ENDIAN);

bbuf.wrap(mplew.getPacket().getBytes());
lbuf.wrap(mplew.getPacket().getBytes());
outputB = HexTool.toString(bbuf.array());
outputL = HexTool.toString(lbuf.array());
%>
