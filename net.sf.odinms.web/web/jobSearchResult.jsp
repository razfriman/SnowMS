...
<%@page import="net.sf.odinms.web.JobSearch"%>
...
<br /><br />
	...
	if (session.getAttribute("jobSearch") == null)
		return;
	JobSearch search = (JobSearch) session.getAttribute("jobSearch");
	...
	%>
		<tr>
			...
			<td>
				<a href="job.jsp?job=<%= job.getId() %>"><%= job.toString() %></a>
			</td>
			<td>
			<img src="IconExtractor?job=<%= job.getId() %>" alt=<%= job.toString() %> />
			</td>
		</tr>
	<%
	}
